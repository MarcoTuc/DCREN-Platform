...
# Modify this to only include PLSNNToolbox
MODULES = {"PLSNNToolbox": TB.modules_display["PLSNN"]}
MODULE_PARAMETERS = {k: v for k, v in TB.module_frontend_parameters().items() if k == "PLSNNToolbox"}
...
class TrainState(rx.State):
    ...
    async def handle_train_model(self):
        self.running = True
        yield
        try:
            runner = Module(
...
def check_data_page():
    return rx.hstack(
            rx.select(
                TrainState.databases,
                placeholder="Select dataset",
                width="40%",
                name='select',
                on_change=TrainState.set_dataset
            ),
            rx.button('Select', width='30%', type="submit"),
            rx.button(
                'Download', 
                on_click=TrainState.db_to_download,
                width='20%', 
                color_scheme='iris',
            ),
...
def varmapper(var) -> rx.Component:
    return rx.hstack(
            ...
            rx.select(
                TrainState._labels,
                width="40%",
                on_change=lambda value: TrainState.set_variable_mapping(var, value),
                value=TrainState.get_variable_mapping[var],  # Use the computed var here
...
def error_dialog():
    return rx.dialog.root(
        rx.dialog.content(
            ...
            rx.dialog.description(TrainState.upload_error),
            rx.cond(
                TrainState.rename_mode,
                rx.vstack(
                    rx.input(
                        placeholder="Enter new file name (without extension)",
                        on_change=TrainState.set_new_file_name,
                        value=TrainState.new_file_name_without_extension,
                    ),
                    rx.text(f"File will be saved as: {TrainState.new_file_name}"),
                    rx.button(
                        "Confirm Rename",
                        on_click=TrainState.handle_rename,
                        disabled=TrainState.rename_button_disabled,
                    ),
                    rx.cond(
                        TrainState.rename_button_disabled,
                        rx.box(
                ...
                rx.vstack(
                    rx.text(f"File '{TrainState.existing_file_name}' already exists."),
                    rx.hstack(
                        rx.button("Change Name", on_click=TrainState.enter_rename_mode),
                        rx.button("Overwrite", on_click=TrainState.handle_overwrite),
                        rx.button("Cancel Upload", on_click=TrainState.handle_cancel_upload),
                    ),
                ),
            ),
        ),
        open=TrainState.show_error_dialog,
    )
    ...
    return rx.vstack(
            rx.hstack(
                ...
                rx.vstack(
                    rx.button(
                        "Upload",
                        on_click=TrainState.handle_upload_stage_1(rx.upload_files(upload_id="upload_data")),
                    ),
                    rx.button(
                        "Clear",
                        on_click=[rx.clear_selected_files("upload_data"), TrainState.handle_cancel_upload],
                        variant="outline",
                    ),
                    rx.cond(
                        TrainState.show_selected_file,
                        rx.text(TrainState.uploaded_file_name)
                    ),
                ),
            ),
            error_dialog(),
            rx.cond(
                TrainState.processdownload,
                rx.box(
                    rx.divider(),
                        ...
                        rx.drawer.portal(
                            rx.drawer.content(
                                rx.vstack(
                                    rx.hstack(
                                        rx.select(
                                            items=TrainState.available_mappings,
                                            placeholder="Select mapping",
                                            on_change=TrainState.set_selected_mapping,
                                            width="20em%",
                                        ),
                                        rx.button(
                                            "update",
                                            on_click=TrainState.update_available_mappings
                                        ),
                                        rx.drawer.close(rx.button("Close"))
                                    ),
                                    rx.cond(
                                        TrainState.mapping_selected,
                                        rx.vstack(
                                            rx.button(
                                                "Delete mapping",
                                                on_click=TrainState.delete_variable_mapping
                                                ),
                                            rx.foreach(
                                                TrainState.selected_mapping,
                                                lambda x: rx.hstack(
                                                            rx.text(x[0]),
                                                            rx.text("-->"),
                                                            rx.text(x[1])
                    ...
                    rx.grid(
                        rx.foreach(
                            TrainState.shown_varmaps,
                            lambda i: varmapper(i)
                        ),
                        rx.hstack(
                            rx.button(
                                "Prev",
                                on_click=TrainState.prev_page,
                            ),
                            rx.text(
                                f"Page {TrainState.varmap_page+1} / {TrainState.totpages}"
                            ),
                            rx.button(
                                "Next",
                                on_click=TrainState.next_page,
                            ),
                        ),
                        rx.hstack(
                            rx.input(
                                placeholder="Name the mapping...",
                                on_change=TrainState.set_savemappingname
                                ),
                            rx.button(
                                "Save mapping",
                                on_click=TrainState.save_variable_mappings,
                                )
                        ),
                        spacing="3",
                        rows="3",
                    ...
                    rx.button(
                        "Cancel Upload",
                        on_click=TrainState.handle_cancel_upload,
                        color_scheme="red",
                        size="md",
                        margin_top="1em",
                    ),
                    rx.button(
                        ...
                        on_click=TrainState.confirm_upload_and_mapping,
                        color_scheme="green",
                        size="lg",
                        margin_top="1em",
                    ),
...
def model_block():
    return rx.box(
            rx.heading("Algorithms", size='5'),
            rx.divider(),
            rx.hstack(
                rx.vstack(
                    rx.text('Select algorithm'),
                    rx.select(
                        TrainState.modules,
                        value = TrainState.module,
                        on_change = lambda x: TrainState.set_module(x),
                    ),
                    width = '15%',
                    margin = '1em'
                ),
                ...
                #     rx.select(
                #         TrainState.funcs,
                #         value = TrainState.func,
                #         on_change = lambda x: TrainState.get_params(x), 
                #         ),
                #     width = '15%',
                #     margin = '1em'
                # ),
                rx.cond(
                    TrainState.hasparams,
                    rx.vstack(
                        rx.text('Insert parameters'),
                            rx.foreach(TrainState.parameters,
                                lambda p: 
                                rx.hstack(
                                    rx.text(p.name),
                    ...
                    rx.text(f"Module {TrainState.module} has no tunable training parameters", margin = '1em'),
                ),
            ),
            style=box_style,
            width = '33%',
...
def results_block():
    return rx.box(
            rx.heading("Training", size='5'),
            rx.divider(),
            rx.vstack(
                rx.input(
                    placeholder="Custom name for the new model",
                    on_blur=TrainState.set_experiment_name,
                    width='100%',
                ),
                training_button_dynamics(),                
                margin='1em',
                width='80%',
                align='center'
            ),
...
def training_button_dynamics():
    return rx.cond(
        TrainState.experiment_name != "",
        rx.button(
            "Train the model",
            on_click=TrainState.handle_train_model,
            color_scheme='blue',
            width='100%',
            loading=TrainState.running
        ),
        rx.button(
            "Train the model",
