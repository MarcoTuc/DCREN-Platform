function DataStruct = TestValidation(DataStruct) % Called from main program 
    % DataStruct -> DataStructPrediction
...
function DataStruct = LoopThroughValidationSet(DataStruct)
    ...
    DataStruct.hyperStruct.numCrossValidation = 1; % Just one term in cross validation
    for k=1:DataStruct.hyperStruct.numCrossValidation % Scroll through cross-validation set
        ...
        DataStruct = PLSPrediction(k,DataStruct); %***********************************
        DataStruct = NNPrediction(k,DataStruct); %***********************************
                return;
    end
